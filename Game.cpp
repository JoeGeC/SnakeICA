...
Game::Game(int screenWidth, int screenHeight) : m_screenWidth(screenWidth), m_screenHeight(screenHeight)
{
...
void Game::CollectableCollision(Sun& s, sf::RenderWindow& window)
{
    for(Collectable& c : m_collectables)
    {
...
void Game::DrawScoreText(sf::RenderWindow& window)
{
    int i = 0;
    for(Snake* s : m_snakes)
    {
...
//        m_gameState = EGameState::eAtMenu;
//
...
//
//    m_gameState = EGameState::eAtMenu;
//    while(m_gameState == EGameState::eAtMenu)
//    {
//
...
//    Menu menu;
//
//    menu.m_gameState = EGameState::eAtMenu;
//    while (m_gameState == EGameState::eAtMenu)
//    {
//        menu.OpenMenu(m_gameState, m_window, m_screenWidth, m_screenHeight);
//    }
//
//    Run(m_window);
//
...
int Game::Run(sf::RenderWindow& window, SColor playerColors[], GameState gameState)
{
    ...
    //create planets
    m_planets.push_back(Planet(750, 750, 20, 0.1, 0.1));
    m_planets.push_back(Planet(500, 500, 15, 0.05, 0.05));
    ...
    {
        m_snakes.push_back(new PlayerSnake("Player" + std::to_string(i + 1), playerColors[i].color));
    }
    for(int i = 0; i < 4 - m_amountOfPlayers; i++)
    {
        m_snakes.push_back(new AISnake("Player" + std::to_string(i + 1 + m_amountOfPlayers), playerColors[i + m_amountOfPlayers].color));
    }

    m_winner = m_snakes[0]; //sets default winner

    // game loop while in game
    while (gameState.GetGameState() == EGameState::eInGame)
    {
        ...
        if(!m_gameOver)
        {
            ...
            for(Snake* s : m_snakes)
            {
                ...
                SnakeCollision();
                WallCollision(s);
                for(Planet planet : m_planets) //checks each planet
                {
            ...
            CollectableCollision(sun, window);
        }
        ...
        {
            planet.DrawPlanet(window, sun.GetPosition());
        }

        sun.DrawSun(window);

        //Draw north border
        window.draw(m_northBorder);
        m_northBorder.setPosition(m_northBorderPos);

        //Draw south border
        window.draw(m_southBorder);
        m_southBorder.setPosition(m_southBorderPos);

        //Draw east border
        window.draw(m_eastBorder);
        m_eastBorder.setPosition(m_eastBorderPos);

        //Draw west border
        window.draw(m_westBorder);
        ...
        if(m_gameOver)
        {
            ...
            if(sf::Keyboard::isKeyPressed(sf::Keyboard::Return))
            {
                ...
                gameState.SetGameState(EGameState::eAtEndScreen); //return to main menu
